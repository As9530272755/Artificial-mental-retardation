...
func main() {
	listens, err := net.Listen("tcp", "127.0.0.1:8011")
	if err != nil {
	...
	for {
		...
		fmt.Printf("%v[%v]客户端登录!\n", time.Now().Format("[2006-01-02 15:04:05]"), conn.RemoteAddr())
		// 如果监听到连接请求并成功以后，
		// 服务器进入下面的go程，
...
func HandleConnect(conn net.Conn) {
	...
	_, _ = conn.Write([]byte("【" + userName + "】欢迎来到『🔞🔞🔞』群聊系统\n"))
	//广播通知。遍历map
	go func() {
		for _, v := range onlineUsers {
			v.NewUser <- []byte("🤵用户【" + userName + "】加入『🔞🔞🔞』群聊系统\n")
		}
	}()
	...
	go func() {
		...
		for {
			...
			case n == 0:
				...
				for _, v := range onlineUsers {
					if thisUser != "" {
						v.NewUser <- []byte("📢📢📢用户【" + thisUser + "】已离开『🔞🔞🔞』群聊系统\n")
					}
	...
	for {
		...
		case <-time.After(time.Second * 60):
			_, _ = conn.Write([]byte("抱歉，由于长时间未发送聊天内容，您已被踢出『🔞🔞🔞』群聊系统\n"))
			thisUser := onlineUsers[conn.RemoteAddr().String()].name
			for _, v := range onlineUsers {
				if thisUser != "" {
					v.NewUser <- []byte("💨🈲🈲用户[" + thisUser + "]由于长时间未发送消息已被踢出『🔞🔞🔞』群聊系统\n")
				}
			}
			delete(onlineUsers, conn.RemoteAddr().String())
			return
